...
function getCallbackInfo(callback, template) {
  return {
    ...
    checkSend:function() {
      ...
      if (!this.sent && modifyCallbackData(template, template)) {
  ...
  try {
    ...
    if (command.execAction) {
      ...
      command.exec = function ServiceStub_exec(args, context) {
        console.log('contentDocument is '+JSON.stringify(context.environment.contentDocument));
