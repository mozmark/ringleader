/*
 * JSONPath with safe mode - based on:
 * JSONPath 0.8.0 - XPath for JSON
 *
 * Copyright (c) 2007 Stefan Goessner (goessner.net)
 * Licensed under the MIT (MIT-LICENSE.txt) licence.
 */
function jsonPath(obj, expr, arg, unsafe) {
  var P = {
    ...
    eval: function(x, _v, _vname) {
      let message = 'script expressions are not enabled';
      try {
        if (unsafe) {
      ...
      throw new SyntaxError("jsonPath: " + message + ": " + x.replace(/@/g, "_v").replace(/\^/g, "_a"));
    }
  };
