...
HeaderModifier.prototype.modify = function(aChannel) {
  for (let idx in this.values) {
    console.log('HeaderModifier: '+this.headerName+' : '+this.values[idx]);
...
function ModifierScopes() {
  ...
  return {
    getScope:function(key) {
      console.log('scope data is '+JSON.stringify(scopes));
      console.log('get scope for '+key);
      if (!key) {
        key = 'global';
      }
      if (!scopes[key]) {
...
MitmProxy.prototype.modify = function (aEvent) {
  ...
  let runModifiers = function(scope) {
    console.log('run modifiers..: '+JSON.stringify(scope));
    for (let type in scope) {
      console.log('running modifiers of type '+type);
  ...
  for(let idx in commands){
    ...
    if (command.command && command.command === 'addToHeader') {
      ...
      if (command.params && command.params.headerName) {
        ...
        let scope = this.modifierScopes.getScope(commandKey);
        ...
        let modifier = headerModifiers[headerName];
        // add the value to the header modifier
