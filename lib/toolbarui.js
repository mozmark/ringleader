...
ProxyRecorder.prototype.modify = function (event) {
    ...
    if(tab && tab._isRecording){
        channel.setRequestHeader('X-Security-Proxy','record', true);
    }
    if(tab && tab._isIntercepting){
        channel.setRequestHeader('X-Security-Proxy','intercept', true);
    }
}
