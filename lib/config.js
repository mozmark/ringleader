...
var ConfigManager = Class({
  extends: EventTarget,
     ...
     */
    clear : function clear(suppress, name) {
...
Setup.chooseConfig = function() {
  ...
  panel.show();
  configs = [];
  for (config of configManager.currentConfigs()) {
    configs[configs.length] = config;
  }
  panel.port.emit("test", {"names" : configs});
  panel.port.on("select", function(evt){
...
Setup.configure = function(url) {
  return Configuration.fromURL(url).then(function(config) {
    if (config && config.manifest) {
      ...
      if (configManager.currentConfigs()) {
        configManager.clear();
      }
