...
ConfigManager.prototype.clear = function() {
  ...
  var certName = config.cert.name;
  var proxyConfig = JSON.parse(storage.storage.originalConfig);
  // remove the cert
  var cert;
  cert = certdb.findCertByNickname(null, certName);
  ...
  if (!storage.storage.current) {
    if (config) {
      if (config.manifest.proxyPAC) {
        ...
        console.log('proxyPAC is '+config.manifest.proxyPAC);
      ...
      if (config.cert && config.cert.der && config.cert.name) {
        certdb.addCert(config.cert.der,'Cu,,',config.cert.name);
      }
    ...
    Setup.userConfirm().then(function () {
      if(manifest.features.CACert) {
        ...
        readURI(certURL).then(function(data){
          ...
          config.cert = {"name":"MITM Test Cert","der":der};
